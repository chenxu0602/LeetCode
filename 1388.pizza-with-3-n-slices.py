#
# @lc app=leetcode id=1388 lang=python3
#
# [1388] Pizza With 3n Slices
#
# https://leetcode.com/problems/pizza-with-3n-slices/description/
#
# algorithms
# Hard (44.95%)
# Likes:    283
# Dislikes: 4
# Total Accepted:    4.7K
# Total Submissions: 10.4K
# Testcase Example:  '[1,2,3,4,5,6]'
#
# There is a pizza with 3n slices of varying size, you and your friends will
# take slices of pizza as follows:
# 
# 
# You will pick any pizza slice.
# Your friend Alice will pick next slice in anti clockwise direction of your
# pick. 
# Your friend Bob will pick next slice in clockwise direction of your pick.
# Repeat until there are no more slices of pizzas.
# 
# 
# Sizes of Pizza slices is represented by circular array slices in clockwise
# direction.
# 
# Return the maximum possible sum of slice sizes which you can have.
# 
# 
# Example 1:
# 
# 
# 
# 
# Input: slices = [1,2,3,4,5,6]
# Output: 10
# Explanation: Pick pizza slice of size 4, Alice and Bob will pick slices with
# size 3 and 5 respectively. Then Pick slices with size 6, finally Alice and
# Bob will pick slice of size 2 and 1 respectively. Total = 4 + 6.
# 
# 
# Example 2:
# 
# 
# 
# 
# Input: slices = [8,9,8,6,1,1]
# Output: 16
# Output: Pick pizza slice of size 8 in each turn. If you pick slice with size
# 9 your partners will pick slices of size 8.
# 
# 
# Example 3:
# 
# 
# Input: slices = [4,1,2,5,8,3,1,9,7]
# Output: 21
# 
# 
# Example 4:
# 
# 
# Input: slices = [3,1,2]
# Output: 3
# 
# 
# 
# Constraints:
# 
# 
# 1 <= slices.length <= 500
# slices.length % 3 == 0
# 1 <= slices[i] <= 1000
# 
# 
#

# @lc code=start
from functools import lru_cache

class Solution:
    def maxSizeSlices(self, slices: List[int]) -> int:
        # @lru_cache(None)
        # def max_size(slices_str):
        #     if not slices_str or slices_str == "":
        #         return 0
        #     slices = list(map(int, slices_str.split(',')))
        #     if len(slices) == 3: return max(slices)

        #     ans = 0
        #     for i in range(len(slices)):
        #         if i == 0:
        #             ans = max(ans, slices[i] + max_size(','.join(map(str, slices[2:-1]))))
        #         elif i == len(slices) - 1:
        #             ans = max(ans, slices[i] + max_size(','.join(map(str, slices[1:-2]))))
        #         else:
        #             ans = max(ans, slices[i] + max_size(','.join(map(str, slices[:i - 1] + slices[i + 2:]))))

        #     return ans

        # return max_size(','.join(map(str, slices)))


        # Move smallest slice of pizza to the end and then do dp to pick slices
        # We can understand the problem: pick n non-adjacent elements from circular array m=3n elements so that the sum of the elements is maximum.
        # We can't pick 0th and (m-1)th elements at the same time, since it's adjacent in circular array.
        # So there are total 2 cases, then we can return the case with maximum sum:
        # Case 1: Don't pick (m-1)th element => Solve problem: pick n non-adjacent elements from linear array elements in ranges [0..m-2] so that the sum of the elements is maximum.
        # Case 2: Don't pick 0th element => Solve problem: pick n non-adjacent elements from linear array elements in ranges [1..m-1] so that the sum of the elements is maximum.
        i = slices.index(min(slices))
        slices = slices[i + 1:] + slices[:i + 1]

        total, n = len(slices), len(slices) // 3
        dp = [[0] * (total + 1) for _ in range(n + 1)]
        for r in range(n):
            for c in range(r, total):
                dp[r][c] = max(slices[c] + dp[r - 1][c - 2], dp[r][c - 1])

        return dp[-2][-2]


        # @lru_cache(None)
        # def dp(i, j, k, cycle=0):
        #     if k == 1: return max(slices[i:j + 1])
        #     if j - i + 1 < k * 2 - 1: return float("-inf")
        #     return max(dp(i + cycle, j - 2, k - 1) + slices[j], dp(i, j - 1, k))
        # return dp(0, len(slices) - 1, len(slices) // 3, 1)


        
# @lc code=end

