        ...
        for i in range(len(code)):
            ...
            if code[i] == '<':
                if self.stack and code[i+1] == '!':
                    closeindex = code.index("]]>", i+1)
                    if closeindex < 0 or not self.isValidCdata(code[i+2:closeindex+1]):
                        return False
                else:
                    if code[i+1] == '/':
                        i += 1
                        ending = True
                    closeindex = code.index('>', i+1)
                    if closeindex < 0 or not self.isValidTagName(code[i+1:closeindex+1], ending):
                        return False
                i = closeindex
