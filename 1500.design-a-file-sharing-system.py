...
class FileSharing:
    ...
    def __init__(self, m: int):
        self.pq, self.user2chunks = [1], defaultdict(int) 

    def join(self, ownedChunks: List[int]) -> int:
        uid = heapq.heappop(self.pq) 
        if not self.pq:
            heapq.heappush(self.pq, uid + 1)
        self.user2chunks[uid] = set(ownedChunks)
        return uid

    def leave(self, userID: int) -> None:
        heapq.heappush(self.pq, userID) 
        self.user2chunks.pop(userID)

    def request(self, userID: int, chunkID: int) -> List[int]:
        res = [] 
        for k, s in self.user2chunks.items():
            if chunkID in s:
                res.append(k)
        if res:
            self.user2chunks[userID].add(chunkID)
