...
class Solution:
    def assignTasks(self, servers: List[int], tasks: List[int]) -> List[int]:
        # Time  complexity: O(len(tasks) x log(len(Servers)))
        # Space complexity: O(S + T)

        res = []
        h1 = [[w, i, 0] for i, w in enumerate(servers)]
        h2 = []
        heapq.heapify(h1)

        for j, task in enumerate(tasks):
            while h2 and h2[0][0] <= j or not h1:
                time, w, i = heapq.heappop(h2)
                heapq.heappush(h1, [w, i, time])

            w, i, time = heapq.heappop(h1)
            res.append(i)
            heapq.heappush(h2, [max(time, j) + task, w, i])
